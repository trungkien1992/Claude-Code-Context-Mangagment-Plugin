...
class ContextMonitor:
    ...
    async def update_token_usage(self, current_tokens: int, token_limit: Optional[int] = None):
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_to_limit': f"{time_to_limit:.1f} minutes" if time_to_limit else "Unknown",
    ...
    async def get_recommendations(self) -> List[str]:
        ...
        if usage_percentage > 85:
            recommendations.append("🚨 Consider /compact --preserve-code to reduce token usage")
        elif usage_percentage > 75:
            recommendations.append("⚠️ Token usage is high, monitor closely")
        
        if self.burn_rate > self.config.get('burn_rate_threshold', 150):
            recommendations.append("📈 High burn rate detected, consider optimizing session")
        ...
        if self.context_switches > 5:
            recommendations.append("🔄 Multiple context switches detected, consider focusing")
    ...
    async def _update_session_metrics(self):
        ...
        if len(self.token_history) >= 2:
            ...
            time_diff = (recent_entries[-1]['timestamp'] - recent_entries[0]['timestamp']).total_seconds() / 60
            
            if time_diff > 0:
                self.burn_rate = token_diff / time_diff
        
        logger.debug(f"Session metrics updated - Tokens: {self.current_tokens}, Burn rate: {self.burn_rate:.1f}")
