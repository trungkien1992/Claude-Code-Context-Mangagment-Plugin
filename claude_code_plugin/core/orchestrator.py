...
class PluginOrchestrator:
    ...
    def _calculate_decision_factors(self, token_usage: Dict, burn_rate: float, 
                                  session_duration: float, session_type: str, 
                                  files_modified: int) -> Dict[str, float]:
    ...
    def _evaluate_factors(self, factors: Dict[str, float]) -> tuple:
        ...
        if factors['token_pressure'] > 0.6 or factors['burn_rate_pressure'] > 0.6:
            return (
                ActionType.MONITOR,
                "Moderate usage - continue monitoring",
                0.6
            )
        ...
        return (
            ActionType.NO_ACTION,
            "All factors within acceptable ranges",
            0.5
        )
    ...
    def _record_decision(self, decision: Dict):
        ...
        logger.debug(f"Decision recorded: {decision['action']} (confidence: {decision['confidence']:.2f})")
    ...
    async def analyze_decision_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'decision_frequency': len(self.decision_history) / max(1, 
                (datetime.now() - datetime.fromisoformat(self.decision_history[0]['timestamp'])).total_seconds() / 3600)
        }
    ...
    def get_current_recommendations(self, session_data: Dict) -> List[str]:
        ...
        token_usage = session_data.get('token_usage', {})
        if token_usage:
            usage_ratio = token_usage.get('current_tokens', 0) / token_usage.get('token_limit', 200000)
            
            if usage_ratio > 0.9:
                recommendations.append("🚨 Critical: Run /compact immediately")
            elif usage_ratio > 0.8:
                recommendations.append("⚠️ High usage: Consider /compact --preserve-code")
