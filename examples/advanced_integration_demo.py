...
from claude_context_management.core.analysis import (
...
from claude_context_management.core.orchestration import (
    OrchestrationEngine,
    CommandType,
    WorkflowConfig
)
from claude_context_management.core.models import (
    SessionSnapshot,
    SessionState,
    TaskDefinition,
    Priority,
...
class AdvancedContextManager:
    ...
    async def _demo_phase_1_setup(self):
        ...
        tasks[1].add_dependency(tasks[0].id)  # Confidence scoring depends on async processing
        tasks[2].add_dependency(tasks[0].id)  # Memory optimization depends on async processing
    ...
    async def _demo_phase_2_burn_rate(self):
        ...
        session_data = {
            'conversation_history': [
                {'content': 'User: Help me optimize my code', 'timestamp': datetime.now() - timedelta(minutes=10)},
                {'content': 'Assistant: I can help you optimize your code for better performance...', 'timestamp': datetime.now() - timedelta(minutes=9)},
                {'content': 'User: What about memory efficiency?', 'timestamp': datetime.now() - timedelta(minutes=5)},
                {'content': 'Assistant: For memory efficiency, consider using deques and sliding windows...', 'timestamp': datetime.now() - timedelta(minutes=4)}
            ],
            'tool_usage': [
                ...
                {'name': 'MultiEdit', 'timestamp': datetime.now() - timedelta(minutes=1)}
            ],
            'file_changes': [
        ...
        logger.info(f"🔥 Dynamic burn rate: {analysis.token_usage.burn_rate:.2f} tokens/minute")
        logger.info(f"⏱️  Time to limit: {analysis.token_usage.time_to_limit:.1f} minutes")
        ...
        await record_token_usage("context_analysis", analysis.token_usage.current_tokens)
    ...
    async def _demo_phase_5_confidence_scoring(self):
        ...
        logger.info(f"📊 Overall confidence: {confidence_assessment.overall_confidence:.3f}")
        logger.info(f"🎯 Confidence level: {confidence_assessment.confidence_level.value}")
        logger.info(f"📈 Reliability score: {confidence_assessment.reliability_score:.3f}")
    ...
    async def _demo_phase_7_reporting(self):
        ...
        for task_id in self.active_tasks:
            ...
            self.current_session.add_achievement(f"Completed: {self.active_tasks[task_id].title}")
        ...
        report = {
            ...
            'task_completion': {
                ...
                'completion_rate': len(self.completed_tasks) / len(self.active_tasks) if self.active_tasks else 0
            },
