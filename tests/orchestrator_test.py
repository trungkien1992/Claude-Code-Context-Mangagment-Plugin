...
from orchestrator_engine import (
    OrchestrationEngine, ContextAnalyzer, DecisionEngine, CommandDispatcher,
    WorkflowConfig, ContextMetrics, CommandType
)
...
class TestDecisionEngine(unittest.TestCase):
    ...
    def test_digest_score_calculation(self):
        ...
        score = self.decision_engine._calculate_digest_score(metrics, "document progress")
    ...
    def test_compact_score_calculation(self):
        ...
        metrics = ContextMetrics(
            token_count=7000,
            repetitive_patterns=4,
            message_count=30,
            topic_changes=3
        )
        
        score = self.decision_engine._calculate_compact_score(metrics, "summarize conversation")
        self.assertGreater(score, 0.5)
    
    def test_command_decision_logic(self):
        """Test command decision logic"""
        # Test digest preference
        with patch.object(self.decision_engine, '_calculate_digest_score', return_value=0.7):
            with patch.object(self.decision_engine, '_calculate_compact_score', return_value=0.4):
                command_type, params = self.decision_engine.decide_command("document session")
                self.assertEqual(command_type, CommandType.DIGEST)
        
        # Test compact preference
        with patch.object(self.decision_engine, '_calculate_digest_score', return_value=0.4):
            with patch.object(self.decision_engine, '_calculate_compact_score', return_value=0.7):
                command_type, params = self.decision_engine.decide_command("compress conversation")
...
class TestCommandDispatcher(unittest.TestCase):
    ...
    def test_execute_hybrid_command(self):
        ...
        with patch.object(self.dispatcher, '_execute_compact', return_value={'status': 'success'}):
            with patch.object(self.dispatcher, '_execute_digest', return_value={'status': 'success'}):
