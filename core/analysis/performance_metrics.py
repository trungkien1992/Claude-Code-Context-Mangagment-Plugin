...
class SlidingWindowAnalyzer:
    ...
    def get_window_data(self, window_minutes: Optional[int] = None) -> List[MetricPoint]:
    ...
    def calculate_stats(self, window_minutes: Optional[int] = None) -> Optional[SlidingWindowStats]:
        """Calculate comprehensive statistics for sliding window"""
        window_size = window_minutes or self.window_minutes
        cache_key = f"stats_{window_size}"
        
        # Check cache
        if (cache_key in self.window_cache and 
            datetime.now() - self.cache_expiry < timedelta(seconds=10)):
    ...
    def _calculate_trend(self, data_points: List[MetricPoint]) -> float:
        ...
        x_values = [(point.timestamp - first_time).total_seconds() / 60 
    ...
    def detect_anomalies(self, threshold_std: float = 2.0) -> List[MetricPoint]:
        ...
        return [point for point in window_data if abs(point.value - stats.mean) > threshold]
...
class PerformanceMetricsEngine:
    ...
    async def record_metric(self, metric_type: MetricType, value: float, 
                          operation_type: str = None, metadata: Dict = None):
        ...
        if analyzer:
            ...
            if operation_type:
                await self._update_performance_profile(operation_type, metric_type, value)
    ...
    async def get_performance_summary(self) -> Dict[str, Any]:
        ...
        for metric_type, analyzer in self.analyzers.items():
            task = asyncio.create_task(
                asyncio.to_thread(analyzer.calculate_stats)
            )
        ...
        for metric_type, analyzer in self.analyzers.items():
            ...
            if anomalies:
                issues.append({
                    ...
                    'recommendation': self._get_recommendation(metric_type, anomalies)
                })
        
        return issues
    
    async def get_performance_profile(self, operation_type: str) -> Optional[PerformanceProfile]:
        """Get performance profile for specific operation type"""
        return self.performance_profiles.get(operation_type)
    
    async def _update_performance_profile(self, operation_type: str, 
        ...
        if not profile:
            ...
            if stats:
                ...
                profile.threshold_metrics[metric_type] = stats.mean + (2 * stats.std_dev)
    ...
    def _get_recommendation(self, metric_type: MetricType, 
                          anomalies: List[MetricPoint]) -> str:
        ...
        return recommendations.get(metric_type, "Monitor metric trends and investigate patterns")
    ...
    async def cleanup_old_data(self, hours_to_keep: int = 24):
        ...
        for analyzer in self.analyzers.values():
            ...
            while (analyzer.data_points and 
                   analyzer.data_points[0].timestamp < cutoff_time):
    ...
    async def export_metrics(self, filename: str, format: str = 'json'):
        ...
        for op_type, profile in self.performance_profiles.items():
            data['profiles'][op_type] = {
                'baseline_metrics': {k.name: v for k, v in profile.baseline_metrics.items()},
                'threshold_metrics': {k.name: v for k, v in profile.threshold_metrics.items()},
