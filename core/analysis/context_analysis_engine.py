...
class ContextAnalysisEngine:
    ...
    def __init__(self, rag_system: Optional[Any] = None, config: Optional[Dict] = None):
        ...
        try:
            ...
            logger.warning("Using approximate token counting (install ticktoken for accuracy)")
    ...
    async def analyze_session_context(self, session_data: Dict) -> ContextAnalysis:
        ...
        try:
            ...
            token_usage, session_metrics, complexity_analysis, session_type = await asyncio.gather(
                self._analyze_token_usage(session_data.get('conversation_history', [])),
            ...
            if self.summarize_old_messages and self._should_generate_summary():
                summary = await self._generate_conversation_summary(self.conversation_history)
            ...
            logger.debug(f"Context analysis completed - Type: {session_type.value} "
    ...
    def _update_session_state(self, session_data: Dict):
        ...
        # Log current states
        logger.debug(f"Message count: {len(self.conversation_history)} "
                  f"Tokens: {self._count_messages_tokens()}")

    async def _analyze_token_usage(self, conversation_history: List[Dict]) -> TokenUsage:
        ...
        time_to_limit = remaining_tokens / max(burn_rate, 0.0001) if burn_rate > 0 else None

        # Determine threshold status
        threshold_reached = max(full_tokens, history_tokens) >= (self.token_limit * self.threshold_ratio)

        # Calculate prediction confidence
        prediction_confidence = min(0.95,
                                   math.sqrt(len(self.token_history) / 100))  # grows with more data

        return TokenUsage(
            current_tokens=full_tokens,
            estimated_total=max(full_tokens, history_tokens) + (burn_rate * 60),  # Project for next hour
            burn_rate=burn_rate,
            time_to_limit=time_to_limit,
            threshold_reached=threshold_reached,
            prediction_confidence=prediction_confidence,
            token_history=list(self.token_history),
            last_update_duration=(datetime.now() - self.session_start_time).total_seconds() / 60
        )
    ...
    def _calculate_dynamic_burn_rate(self) -> float:
        ...
        if len(window_data) < 2:
            ...
            session_duration = max(0.01, (now - self.session_start_time).total_seconds() / 60)
            return max(0, self._count_messages_tokens(self.conversation_history)) / session_duration
    ...
    async def _analyze_session_metrics(self, session_data: Dict) -> SessionMetrics:
        ...
        file_modifications = len([change for change in file_changes if change.get('type') in ('M', 'A')])
    ...
    async def _classify_session_type(self, tool_usage: List[Dict]) -> SessionType:
        ...
        edit_ratio = (tool_counts.get('Edit', 0) + tool_counts.get('MultiEdit', 0)) / total_tools
        read_ratio = tool_counts.get('Read', 0) / total_tools
        bash_ratio = tool_counts.get('Bash', 0) / total_tools
        search_ratio = (tool_counts.get('Grep', 0) + tool_counts.get('Glob', 0)) / total_tools
    ...
    def _generate_recommendations(self, token_usage: TokenUsage, session_metrics: SessionMetrics, 
                                task_complexity: TaskComplexity, session_type: SessionType) -> List[str]:
        ...
        if token_usage.time_to_limit and token_usage.time_to_limit < 15:
            recommendations.append("Critical: Token limit approaching, immediate action needed")
            
        # Session-based recommendations
        if session_metrics.duration_minutes > 60:
            recommendations.append("Long session detected - consider /digest --snapshot")
        ...
        if task_complexity.level == ComplexityLevel.CRITICAL:
            recommendations.append("Critical complexity - consider breaking into smaller tasks")
        elif task_complexity.level == ComplexityLevel.HIGH:
            recommendations.append("High complexity - document progress frequently")
            
        # Type-specific recommendations
        if session_type == SessionType.CODING and session_metrics.file_modifications > 10:
            recommendations.append("Many files modified - consider commit checkpoint")
        elif session_type == SessionType.DEBUGGING and session_metrics.duration_minutes > 30:
            recommendations.append("Extended debugging - document findings")
            
        return recommendations

    def _calculate_confidence_score(self, token_usage: TokenUsage, session_metrics: SessionMetrics,
    ...
    def _trim_conversation_history(self):
        ...
        while (self.conversation_history and 
               self.conversation_history[0].get('analysis_timestamp', now) < cutoff_time):
    ...
    def _calculate_tool_complexity(self, tool_usage: List[Dict]) -> float:
        ...
        complex_count = sum(1 for tool in tool_usage if tool.get('name') in complex_tools)
