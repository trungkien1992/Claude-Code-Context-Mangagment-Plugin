...
class RAGConnector:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"RAG system health check failed: {response.status_code}")
                return False
    ...
    async def index_document(self, document: RAGDocument) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Document indexing failed: {response.status_code} - {response.text}")
                return False
    ...
    async def index_batch(self, documents: List[RAGDocument]) -> int:
        ...
        logger.info(f"📚 Batch indexing complete: {successful_indexes}/{len(documents)} documents indexed")
    ...
    async def delete_document(self, document_id: str, collection_name: str = "claude_context_management") -> bool:
...
class ContextRAGManager:
    ...
    async def index_session_context(self, 
                                   session_id: str, 
        ...
        if 'key_implementations' in context_data:
            content_parts.append(f"Key Implementations: {context_data['key_implementations']}")
        
        if 'technical_decisions' in context_data:
            content_parts.append(f"Technical Decisions: {context_data['technical_decisions']}")
        
        if 'performance_metrics' in context_data:
            content_parts.append(f"Performance Metrics: {context_data['performance_metrics']}")
    ...
    async def search_similar_sessions(self, 
                                    query: str, 
                                    session_type: str = None,
                                    max_results: int = 5) -> List[RAGResult]:
...
async def get_similar_sessions(query: str, max_results: int = 5) -> List[RAGResult]:
    ...
    return await context_rag_manager.search_similar_sessions(query, max_results=max_results)