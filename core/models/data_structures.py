...
class ContextWindow(BaseModel):
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        if entry_count > 1:
            timestamps = [datetime.fromisoformat(e['timestamp']) for e in recent_entries]
...
class PerformanceProfile(BaseModel):
    ...
    def record_execution(self, result: ExecutionResult):
        ...
        for metric_name, value in result.performance_metrics.items():
            ...
            if (metric_name not in self.best_performance or 
                value < self.best_performance[metric_name]):
                self.best_performance[metric_name] = value
                
            if (metric_name not in self.worst_performance or 
                value > self.worst_performance[metric_name]):
