...
class ContextAnalyzer:
    ...
    async def analyze_session(self) -> ContextMetrics:
        ...
        self.metrics.session_duration = (datetime.now() - self.session_start).total_seconds() / 60
        ...
        try:
            ...
            if claude_log.exists():
                with open(claude_log, 'r') as f:
                    ...
                    return len([line for line in lines if line.strip().startswith('user:') or line.strip().startswith('assistant:')]) // 2
        except:
            pass
    ...
    def _count_completed_tasks(self) -> int:
        ...
        try:
            ...
            digest_dir = Path('/Users/admin/AstraTrade-Project/claude-rag/digest_resource')
        ...
        try:
            ...
            if claude_log.exists():
                with open(claude_log, 'r') as f:
                    ...
                    repetitive_count = sum(1 for count in word_counts.values() if count > 10)
        ...
        try:
            ...
            if claude_log.exists():
                with open(claude_log, 'r') as f:
                    ...
                    topic_keywords = ['implement', 'create', 'debug', 'fix', 'analyze', 'explain']
                    topic_changes = 0
                    lines = content.split('\n')
    ...
    def _calculate_dynamic_burn_rate(self) -> float:
        ...
        window_data = [(ts, tokens) for ts, tokens in self.token_history 
                      if ts >= window_start]
...
class DecisionEngine:
    ...
    async def decide_command(self, user_intent: str = "") -> Tuple[CommandType, Dict[str, Any]]:
        """Async decision making with parallel analysis"""
        # Parallel execution of analysis and RAG context
        tasks = [
            self.analyzer.analyze_session(),
            self._get_rag_context_async(user_intent) if self.rag_available else asyncio.create_task(asyncio.sleep(0, result={}))
        ]
        
        metrics, rag_context = await asyncio.gather(*tasks)
        
        # Parallel score calculation
        digest_score_task = asyncio.create_task(self._calculate_digest_score_async(metrics, user_intent, rag_context))
        compact_score_task = asyncio.create_task(self._calculate_compact_score_async(metrics, user_intent, rag_context))
        
        digest_score, compact_score = await asyncio.gather(digest_score_task, compact_score_task)
    ...
    def _calculate_digest_score(self, metrics: ContextMetrics, user_intent: str, rag_context: Dict = None) -> float:
        ...
        # User intent factor
        if any(keyword in user_intent.lower() for keyword in ['history', 'previous', 'session', 'document']):
            score += 0.3
        ...
        if rag_context:
            ...
            if any('digest' in session.get('actions', []) for session in similar_sessions):
                score += 0.1
        
        return min(1.0, score)
    
    def _calculate_compact_score(self, metrics: ContextMetrics, user_intent: str, rag_context: Dict = None) -> float:
        ...
        # User intent factor
        if any(keyword in user_intent.lower() for keyword in ['summarize', 'compress', 'clarify', 'focus']):
            score += 0.4
        ...
        if rag_context:
            ...
            similar_sessions = rag_context.get('similar_sessions', [])
            if any('compact' in session.get('actions', []) for session in similar_sessions):
                score += 0.1
    ...
    def _get_compact_params(self, metrics: ContextMetrics) -> Dict[str, Any]:
        ...
        if metrics.token_count / 8000 > 0.8:
            return {'level': 'deep', 'preserve_code': True, 'description': 'Token optimization'}
        elif metrics.repetitive_patterns > 3:
            return {'level': 'standard', 'focus': 'clarity', 'description': 'Pattern reduction'}
        else:
            return {'level': 'light', 'preserve_code': True, 'description': 'Light optimization'}
    ...
    async def _calculate_digest_score_async(self, metrics: ContextMetrics, user_intent: str, rag_context: Dict = None) -> float:
        """Async version of digest score calculation"""
        return await asyncio.to_thread(self._calculate_digest_score, metrics, user_intent, rag_context)
    
    async def _calculate_compact_score_async(self, metrics: ContextMetrics, user_intent: str, rag_context: Dict = None) -> float:
        """Async version of compact score calculation"""
        return await asyncio.to_thread(self._calculate_compact_score, metrics, user_intent, rag_context)
...
class CommandDispatcher:
    ...
    async def execute_command_async(self, command_type: CommandType, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def execute_command(self, command_type: CommandType, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_digest(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        digest_script = Path('/Users/admin/AstraTrade-Project/claude-rag/src/digest_implementation.py')
        ...
        if action == 'create':
            ...
            return self._run_command(['python', str(digest_script), '--create'])  # Update via create
        elif action == 'show':
...
class OrchestrationEngine:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = config_path or Path.home() / '.claude' / 'orchestrator_config.yaml'
    ...
    async def orchestrate(self, user_intent: str = "", force_command: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if force_command:
                ...
                command_type, params = await self.decision_engine.decide_command(user_intent)
            
            # Execute command and generate recommendations in parallel
            command_task = asyncio.create_task(self.dispatcher.execute_command_async(command_type, params))
            recommendations_task = asyncio.create_task(self._generate_recommendations_async(metrics))
            
            result, recommendations = await asyncio.gather(command_task, recommendations_task)
    ...
    def _get_default_params(self, command_type: CommandType, metrics: ContextMetrics) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, metrics: ContextMetrics) -> List[str]:
        ...
        if metrics.task_completion_count >= 3:
            recommendations.append("Good time to run /digest --create to document progress")
        
        if metrics.session_duration > 60:
            recommendations.append("Long session detected - consider /digest --snapshot for archival")
        
        if metrics.repetitive_patterns > 4:
            recommendations.append("Repetitive patterns detected - /compact --deep might help")
        
        if metrics.complexity_score > 0.8:
            recommendations.append("High complexity session - hybrid approach recommended")
        
        return recommendations
    
    async def _generate_recommendations_async(self, metrics: ContextMetrics) -> List[str]:
        """Async version of recommendations generation"""
        return await asyncio.to_thread(self._generate_recommendations, metrics)
    
    def _log_session(self, metrics: ContextMetrics, command_type: CommandType, params: Dict, result: Dict):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_commands': self.dispatcher.command_log[-5:] if self.dispatcher.command_log else []
        }
...
async def main_async():
    ...
    parser.add_argument('--force', type=str, choices=['digest', 'compact', 'hybrid'], help='Force specific command')
    ...
    if args.orchestrate:
        ...
        profiles = {
            'dev': {'automation_level': 'high', 'digest_threshold': 2, 'continuity_priority': 'high'},
            'research': {'automation_level': 'medium', 'token_threshold': 0.8, 'digest_threshold': 4},
            'debug': {'automation_level': 'low', 'token_threshold': 0.7, 'digest_threshold': 5},
            'doc': {'automation_level': 'high', 'digest_threshold': 1, 'continuity_priority': 'high'}
        }
