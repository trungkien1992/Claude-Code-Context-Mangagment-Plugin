...
class DecisionContext:
    ...
    def recent_decision_count(self) -> int:
        ...
        return len([d for d in self.previous_decisions
                    if (datetime.now() - d.metadata.get('timestamp', datetime.min)).seconds < 300])
...
class DecisionEngine:
    ...
    * Resilience to uncertain outcomes
    ...
    def __init__(self, context_analyzer: ContextAnalysisEngine, config: ClaudeRAGConfig):
        ...
        logger.info("Decision Engine initialized with %d rules",
                   len(self.decision_rules))

    def _load_configuration(self):
        """Load all configuration values from config object"""
        # Decision parameters
        decision_cfg = getattr(self.config, 'decision', {})
        self.min_decision_interval = decision_cfg.get('min_decision_interval', 5)  # minutes
    ...
    async def make_orchestration_decision(self, session_data: Dict) -> OrchestrationDecision:
        """
    ...
    async def _evaluate_decision_rules(self, context: DecisionContext) -> List[Tuple[DecisionRule, float]]:
        ...
        rule_results.sort(key=lambda x: (x[0].priority if x[0].is_critical else x[0].priority-10,
                                       x[1]), reverse=True)

        logger.debug(f"Evaluated {len(rule_results)} active rules out of {len(self.decision_rules)} total")
        return rule_results

    def _get_boolean_rules(self) -> List[DecisionRule]:
        """Return all rules that implement threshold tests"""
        return [rule for rule in self.decision_rules if "threshold" in rule.name.lower()]

    def _get_probabilistic_rules(self) -> List[DecisionRule]:
        """Return all rules that implement graded evaluations"""
        return [rule for rule in self.decision_rules if rule.name not in [
            r.name for r in self._get_boolean_rules()
        ]]

    def _are_dependencies_met(self, rule: DecisionRule, context: DecisionContext) -> bool:
    ...
    async def _evaluate_rule_condition(self, rule: DecisionRule, context: DecisionContext) -> float:
    ...
    def _get_adjusted_rule_weight(self, rule: DecisionRule, context: DecisionContext) -> float:
        ...
        if (rule.name in self.performance_model and
            self.performance_model[rule.name].get('failure_count', 0) > 3):
            return 0.7
    ...
    async def _select_best_decision(
        self,
        rule_results: List[Tuple[DecisionRule, float]],
        ...
        should_execute = (
            confidence.value > self.confidence_threshold and
            self._should_execute_immediately(best_rule, best_score, context)
        )
        ...
        decision = OrchestrationDecision(
            ...
            metadata={
                ...
                'rule_dependencies_met': all(self._are_dependencies_met(dep_rule, context)
    ...
    async def _optimize_for_performance(
        ...
        if self.last_decision_time and not self._is_critical_decision(decision):
            time_since_last = (datetime.now() - self.last_decision_time).total_seconds() / 60
            ...
            if time_since_last < self.min_decision_interval:
                ...
                if (context.system_state.get('token_pressure', False) and
                    decision.primary_command in [CommandType.COMPACT_DEEP]):
        ...
        if perf_data and perf_data.get('success_rate', 0) < 0.7:
            decision.confidence = max(DecisionConfidence.LOW,
    ...
    async def _generate_secondary_commands(
        ...
        combinations = {
            ...
            CommandType.HYBRID_TRANSITION:
                [CommandType.COMPACT_SUMMARIZE, CommandType.DIGEST_SHOW],

            CommandType.HYBRID_IMPLEMENTATION:
                [CommandType.DIGEST_CREATE, CommandType.COMPACT_PRESERVE_CODE],

            CommandType.HYBRID_RESEARCH:
                [CommandType.COMPACT_SUMMARIZE, CommandType.DIGEST_QUERY],
        ...
        if (context.has_token_pressure or
            context.context_analysis.token_usage.time_to_limit < 10):
            if CommandType.DIGEST_QUERY in combinations.get(primary_command, []):
                combinations[primary_command] = [
                    cmd for cmd in combinations[primary_command]
